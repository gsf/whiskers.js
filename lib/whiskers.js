...
whiskers.expand = function(template, partials) {
  var getPartial = function(partials, key, indent) {
    ...
    var keys = key.split('.');
    var partial = getPartialRecurse(partials, keys);
    // drop trailing newlines and prepend indent to each line
    return indent + partial.replace(/\n+$/, '').replace(/\n/g, indent || '\n');
  }; 
  var expandRecurse = function(template, partials) {
    return template.replace(/(\n[ \t]+|)(\\*){>([\w.]+)}/g, function(str, indent, escapeChar, key) {
      ...
      return expandRecurse(getPartial(partials, key, indent), partials);
    });
  };
...
whiskers.compile = function(template) {
  ...
  template = template.replace(/(\n[ \t]*|)(\\*){(\/?)(for|if|)( +|)(not +|)((\w+) +in +|)([\w.]+|)}(?:([ \t]*(?=\n))|)/g, function(str, newlineBefore, escapeChar, close, statement, statementSpace, ifNot, forIn, iterVar, key, newlineAfter, offset, s) {
    ...
    if (statement && statementSpace && key) {
      ...
      if (statement === 'for' && iterVar) {
        return newlineBefore+'\';var '+iterVar+'A=a(c,\''+key+'\');for(var '+iterVar+'I=0;'+iterVar+'I<'+iterVar+'A.length;'+iterVar+'I++){c[\''+iterVar+'\']='+iterVar+'A['+iterVar+'I];b+=\'';
      }
  ...
  // c is context, b is buffer, v is getValue, a is getArray
  // r is recurse, o is object, k is key, s is splitKey, f is first
  return '(function(){return function(c){var v=function(o,k){var r=function(o,s){if(s.length===0){if(typeof o===\'function\')return o();return o}var f=s.shift();if(!o.hasOwnProperty(f))return \'\';return r(o[f],s)};var s=k.split(\'.\');return r(o,s)};var a=function(o,k){return v(o,k)||[]};var b=\''+template+'\';return b}})()';
};
