...
(function(whiskers) {
  ...
  whiskers.compile = function(template) {
    ...
    template = template.replace(/(\\n[ \t]*|)(\\*){(\/?)(for|if|)( +|)(not +|)((\w+) +in +|)([\w.]+|)}(?:([ \t]*(?=\\n))|)/g, function(str, newlineBefore, escapeChar, close, statement, statementSpace, ifNot, forIn, iterVar, key, newlineAfter, offset, s) {
      ...
      if (statement && statementSpace && key) {
        ...
        if (statement === 'for' && iterVar) {
          return newlineBefore+'\';var '+iterVar+'A=g(\''+key+'\');for(var '+iterVar+'I=0,'+iterVar+'L='+iterVar+'A.length;'+iterVar+'I<'+iterVar+'L;'+iterVar+'I++){c[\''+iterVar+'\']='+iterVar+'A['+iterVar+'I];b+=\'';
        }
    ...
    // c is context, g is get, k is key, v is value, b is buffer
    return new Function('c', 'var g=function(k){var v;try{v=eval(\'c.\'+k)||\'\'}catch(e){v=\'\'}if(typeof v==\'function\')v=v();return v;};var b=\''+template+'\';return b');
  };
