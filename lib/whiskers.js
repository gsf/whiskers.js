...
var whiskers = (function() {
  ...
  var getPartial = function(partials, key) {
    // drop trailing newlines in partial
    return getValue(partials, key).replace(/[\n|\r|\r\n]+$/, '');
  };

  // expand all partials
  // XXX expanding partials at compile time means recursion is impossible
  //     but i think it's worth it to have them compiled
  var expand = function(template, partials, usedKeys) {
    var partial;
    usedKeys = usedKeys || {};
    template = template.replace(/(\\*){>([\w.]+)}/g, function(str, escapeChar, key) {
      ...
      if (usedKeys[key]) {
        throw new Error('Whiskers doesn\'t support partials recursion.');
      }
      usedKeys[key] = true;
      // recurse to expand any partials in this partial
      return expand(getPartial(partials, key), partials, usedKeys);
    });
    return template;
  };
    ...
    template = template.replace(/((?:\n|\r|\r\n)[ \t]*|)(\\*){(\/?)(for|if|)( +|)(not +|)((\w+) +in +|)([\w.]+|)}(?:([ \t]*(?=\n|\r|\r\n))|)/g, function(str, newlineBefore, escapeChar, close, statement, statementSpace, ifNot, forIn, iterVar, key, newlineAfter, offset, s) {
      ...
      if (close && statement) {
        if (levels[statement] === 0) {
          console.warn('extra {/'+statement+'} ignored at "'+s.slice(offset-30, offset+10)+'"');
          return '';
        } else {
          levels[statement]--;
