...
(function(whiskers) {
  ...
  // helper function for getting partials
  whiskers.getPartial = function(partials, key) {
    var parts = key.split('.');
    partials = partials[parts.shift()];
    if (!partials || !parts.length) return partials || '';
    key = parts.join('.');
    return whiskers.getPartial(partials, key);
  };
  ...
  whiskers.expand = function(template, partials, indent) {
    ...
    return template.replace(/(\n[ \t]+|)(\\*){>([\w.]+)}/g, function(str, indent, escapeChar, key) {
      ...
      // get partial
      template = whiskers.getPartial(partials, key);
  ...
  whiskers.compile = function(template) {
    ...
    return new Function('c', 'var g=function(o,k){var p=k.split(\'.\');o=o[p.shift()];if(!o||!p.length){if(typeof o==\'function\')o=o();return o||\'\'}k=p.join(\'.\');return g(o,k)};var b=\''+template+'\';return b');
  };
