  ...
  whiskers.__express = function(path, options, fn) {
    ...
    function read(path, fn) {
      ...
      require('fs').readFile(path, 'utf8', function(err, str){
        ...
        if (options.cache) whiskers.cache[path] = str;
        fn(null, str);
      });
    }
    ...
    function render() {
      try {
        fn(null, whiskers.render(view, options));
      } catch (err) {
        fn(err);
      }
    };
    ...
    keys.forEach(function(key){
      read(require('path').join(options.settings.views, options.partials[key]), function(err, str){
        if (err) return fn(err);
