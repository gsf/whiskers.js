...
(function(whiskers) {
  ...
  whiskers.compile = function(template) {
    ...
    template = template.replace(/(\\n[ \t]*|)(\\*){(\/?)(for|if|else|)( +|)(not +|)((\w+) +in +|)([\w.]+|)}(?:([ \t]*(?=\\n))|)/g, function(str, newlineBefore, escapeChar, close, statement, statementSpace, ifNot, forIn, iterVar, key, newlineAfter, offset, s) {
      ...
      // else tag ({else})
      if (statement == 'else') {
        // only allow one else in each if block
        block = stack[stack.length-1];
        if (block && block.statement == 'if' && !block.elseFlag) {
          stack[stack.length-1].elseFlag = true;
          return newlineBefore+'\';}else{b+=\'';
        } 
      ...
      if (close && statement) {
        block = stack[stack.length-1];
        if (block && block.statement == statement) {
          stack.pop();
    ...
    return new Function('c', 'var g=function(o,k){var a=k.split(\'.\');for(var i=0,l=a.length;i<l;i++){o=o[a[i]];if(!o)break}if(typeof o==\'function\')o=o();return o||\'\'};var b=\''+template+'\';return b');
  };
