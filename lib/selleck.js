...
var selleck = (function() {
  ...
  //var iter = function(context, iterVar, arrKey, section) {
  //  var arr = getValue(context, arrKey);
  //  // empty return on missing array
  //  if (!arr) return '';
  //  var retStr = ''
  //  for (var i = 0; i < arr.length; i++) {
  //    context[iterVar] = arr[i];
  //    retStr += eval(expand(section, context));
  //  }
  //  return retStr;
  //};
  //var getPartial = function(partials, partialKey) {
  //  var partial = getValue(globalPartials, partialKey);
  //  // empty return on missing partial
  //  if (!partial) return '';
  //  return eval(expand(partial, context));
  //};
  var expand = function(section, context) {
    //section = section.replace(/\\/, '\\\\');
    section = section.replace(/\n/g, '\\n');
    section = section.replace(forTag, function(str, iterVar, arrKey) {
      return '\';iter(context, \''+iterVar+'\', \''+arrKey+'\') {';
    });
    section = section.replace(forClose, function(str) {
      return '}();buffer+=\'';
    });
    section = section.replace(partialTag, function(str, partialKey) {
      return '\'+getPartial(context, \''+partialKey+'\')+\'';
    });
    return 'function() {var buffer=\''+section+'\';return buffer}';
    //var lines = section.split('\n');
    ////var inSection = false;
    //for (var i = 0; i < lines.length; i++) {
    //  lines[i] = lines[i].replace(forTag, function(str, iterVar, arrVar) {
    //    //inSection = true;
    //    return '\'+iter(context, \''+iterVar+'\', \''+arrVar+'\', \'';
    //  });
    //  lines[i] = lines[i].replace(forClose, function(str) {
    //    //inSection = false;
    //    return '\')+\'';
    //  });
  ...
  var compile = function(template, partials, filters) {
    // first expand all partials
    ...
    //  return '\';iter(context, \''+iterVar+'\', \''+arrKey+'\') {';
    //});
    //template = template.replace(forClose, function(str) {
    //  return '}();buffer+=\'';
    //});
  ...
  var render = function(template, context, partials, filters) {
    return eval(compile(template, context, partials, filters));
