...
var selleck = (function() {
  ...
  var ifNot = /{^if}/;
  var partialTag = /{>([\w.]+)}/;
  var getValue = function(context, key) {
    var gv = function(context, keys) {
      if (keys.length === 0) {
        return context;
      }
      first = keys.shift();
      ...
      return gv(context[first], keys);
    };
    var keys = key.split('.');
    return gv(context, keys);
  }; 
  var iter = function(context, iterVar, arrKey, section) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      //console.log(context);
      retStr += eval(compile(section));
    }
    return retStr;
  };
  var compile = function(section, context) {
  ...
  var render = function(template, context, partials, filters) {
    return eval(compile(template, context));
