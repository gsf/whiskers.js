...
var selleck = (function() {
  ...
  var compile = function(template, partials) {
    ...
    var keyTag, replacement;
    template = template.replace(/(\n[ \t]*|){(\^?)(\/?)(for|if|)( +|)( +(\w+) +in +|)([\w.]+|)}(\s*\n|)/g, function(str, newlineBefore, not, close, statement, ifSpace, forClause, iterVar, key, newlineAfter) {
      if (!not && !close && !statement && !ifSpace && !forClause && key) {
      ...
      if (statement === 'for' && iterVar && key) { // for tag
      ...
      } else if (statement === 'if' && key) { // if tag
        replacement = newlineBefore+'\';if('+(not?'!':'')+'getValue(context,\''+key+'\')){buffer+=\''+newlineAfter;
      } else if (close && statement) { // close tag
    ...
    //var forCount = forCloseCount = ifCount = ifCloseCount = 0;
    //template = template.replace(forTag, function(str, newlineBefore, iterVar, arrKey, newlineAfter) {
    //  forCount++;
    //  if (newlineBefore && newlineAfter) {
    //    newlineBefore = '\n';
    //    newlineAfter = '';
    //  }
    //  return newlineBefore+'\';var '+iterVar+'A=getArray(context,\''+arrKey+'\');for(var '+iterVar+'I=0;'+iterVar+'I<'+iterVar+'A.length;'+iterVar+'I++){context[\''+iterVar+'\']='+iterVar+'A['+iterVar+'I];buffer+=\''+newlineAfter;
    //});
    //template = template.replace(ifTag, function(str, newlineBefore, not, ifVar, newlineAfter) {
    //  ifCount++
    //  if (newlineBefore && newlineAfter) {
    //    newlineBefore = '\n';
    //    newlineAfter = '';
    //  }
    //  return newlineBefore+'\';if('+(not?'!':'')+'getValue(context,\''+ifVar+'\')){buffer+=\''+newlineAfter;
    //});
    //template = template.replace(closeTag, function(str, newlineBefore, statement, newlineAfter) {
    //  if (statement === 'for') forCloseCount++;
    //  if (statement === 'if') ifCloseCount++;
    //  if (newlineBefore && newlineAfter) {
    //    newlineBefore = '\n';
    //    newlineAfter = '';
    //  }
    //  return newlineBefore+'\';}buffer+=\''+newlineAfter;
    //});
    //// some basic error checking
    //if (forCount > forCloseCount) {
    //  throw(new Error('unclosed "for" tag in template'));
    //} else if (forCount < forCloseCount) {
    //  throw(new Error('extra "for" close tag in template'));
    //} else if (ifCount > ifCloseCount) {
    //  throw(new Error('unclosed "if" tag in template'));
    //} else if (ifCount < ifCloseCount) {
    //  throw(new Error('extra "if" close tag in template'));
    //}
    //template = template.replace(keyTag, function(str, key) {
    //  return '\'+getValue(context,\''+key+'\')+\'';
    //});
  ...
  var render = function(template, context, partials) {
    var expanded = expand(template, partials);
    // XXX hash template and partials for cache lookup key?
    var cached = cache[expanded];
    if (!cached) {
      var compiled = compile(template, partials);
      cache[expanded] = eval(compiled);
      cached = cache[expanded];
    } 
    //console.log(cache);
