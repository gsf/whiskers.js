...
var selleck = (function() {
  ...
  var expand = function(template, partials) {
    // expand all partials
    template = template.replace(partialTag, function(str, spaceBefore, partialKey, newlineAfter) {
      if (spaceBefore && newlineAfter) {
        spaceBefore = '\n';
        newlineAfter = '';
      }
      return spaceBefore+getValue(partials, partialKey)+newlineAfter;
    });
    return template;
  };
  var compile = function(template, partials) {
    ...
    template = template.replace(forTag, function(str, spaceBefore, iterVar, arrKey, newlineAfter) {
      forCount++;
      if (spaceBefore && newlineAfter) {
        spaceBefore = '\n';
        newlineAfter = '';
      }
      return spaceBefore+'\';var '+iterVar+'A=getArray(context,\''+arrKey+'\');for(var '+iterVar+'I=0;'+iterVar+'I<'+iterVar+'A.length;'+iterVar+'I++){context[\''+iterVar+'\']='+iterVar+'A['+iterVar+'I];buffer+=\''+newlineAfter;
    });
    template = template.replace(ifTag, function(str, spaceBefore, not, ifVar, newlineAfter) {
      ifCount++
      if (spaceBefore && newlineAfter) {
        spaceBefore = '\n';
        newlineAfter = '';
      }
      return spaceBefore+'\';if('+(not?'!':'')+'getValue(context,\''+ifVar+'\')){buffer+=\''+newlineAfter;
    });
    template = template.replace(closeTag, function(str, spaceBefore, statement, newlineAfter) {
      ...
      return spaceBefore+'\';}buffer+=\''+newlineAfter;
    });
